...
function Pollroute(props) {
    ...
    function confirm() {
        axios.post(url+"/api/deletepoll", qs.stringify({ id: poll._id }), {
            headers: {
    ...
    function voteHandler() {
        ...
        axios.post(url+"/api/votehandler", qs.stringify({ pollId: match.params.id, option }), config)
            .then(res => {
                if (res.data.error)
                    message.error(res.data.error);
                else {
    ...
    useEffect(() => {
        axios({
            method: 'get', url: "http://localhost:5000/api/".concat(match.params.id), headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
                'Authorization': 'Bearer ' + localStorage.getItem("token")
            }
        })
